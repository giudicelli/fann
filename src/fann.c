...
static void fann_run_neuron(struct fann * ann, struct fann_layer *layer_it, struct fann_neuron *neuron_it) {
	struct fann_neuron *neurons, **neuron_pointers;
	unsigned int i, num_connections;
	fann_type *weights;
	unsigned int activation_function;
	fann_type steepness;
	fann_type neuron_sum;
...
FANN_EXTERNAL fann_type *FANN_API fann_run(struct fann * ann, fann_type * input)
{
	...
	unsigned int i, num_input, num_output, num_neurons;
	fann_type *output;
	...
	num_input = ann->num_input;
        ...
        #pragma omp parallel for if(num_input > 1000) schedule(static) private(i)
	for(i = 0; i < num_input; i++)
	{
#ifdef FIXEDFANN
		if(fann_abs(input[i]) > ann->multiplier)
		{
			printf
				("Warning input number %d is out of range -%d - %d with value %d, integer overflow may occur.\n",
				 i, ann->multiplier, ann->multiplier, input[i]);
		}
#endif
...
#ifdef FIXEDFANN
	(ann->first_layer->last_neuron - 1)->value = ann->multiplier;
#else
	...
	for(layer_it = ann->first_layer + 1; layer_it != last_layer; layer_it++)
	{
		...
		for(i = 0; i < num_neurons; i++) {
		    fann_run_neuron(ann, layer_it, &layer_it->first_neuron[i]);
		}
	}
												   ...
												   fann_type max_weight)
{
	...
	{
		weights[i] = (fann_type) (fann_rand(min_weight, max_weight));
	}
